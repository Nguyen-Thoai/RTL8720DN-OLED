...
void setup(){
  ...
  display.println("Test Wifi");
  display.setCursor(26, 36);
  display.println("2.4GHz 5.8GHz"); 
  display.display();
  delay(3000);
  display.clearDisplay();
  display.setTextSize(1);
  ...
  display.println("MADE IN VIETNAM");
  display.display();
  DEBUG_SER_INIT();
  ...
  SSIDCh = scan_results[0].channel >= 36 ? "5.8GHz" : "2.4GHz";
}
int attackstate = 0;
int menustate = 0;
...
void drawssid(){
  while(true){
    ...
    if(digitalRead(BTN_UP)==LOW){
      ...
      SSIDCh = scan_results[scrollindex].channel >= 36 ? "5.8GHz" : "2.4GHz";
    }
    else{
      BPT = 0;
      if(held){
